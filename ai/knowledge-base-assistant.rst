...
knowledge base answers for incoming tickets and draft new ones from the
...
:doc:`AI provider <provider>` with the **Semantic search** capability before
activating the feature. Otherwise, Zammad shows a warning message.
...
Here goes the individual provider.
...
Knowledge Base Answer Suggestions
   ...
   .. warning::
      ...
      Agents should check the answer before sending the suggested article to
      the customer. Even a high relevance doesn't necessarily mean that it's
      content solves the customer's issue.
...
Relevance Score Threshold
   ...
   list of suggestions. The default is ``86 %``. Lower values show more
   suggestions (and more false positives); higher values show only strong
   matches.

After the configuration is done, activate the feature by turning on the switch
